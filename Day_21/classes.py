...
class Statistics:
    ...
    def freq_dist(self):
        ...
        return sorted(
            [(round((count / total_count) * 100, 1), value) for value, count in frequency.items()], 
            reverse=True
        )
...
class PersonAccount:
    ...
    def account_info(self):
        ...
        expenses_str = ', '.join([f"{desc}: {amount}" for desc, amount in self.expenses])
        return (f"Nom: {self.firstname} {self.lastname}\n"
