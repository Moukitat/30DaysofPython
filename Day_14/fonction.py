...
resultat = reduce(
    lambda acc, c: acc + c + ', ',
    filter(lambda c: 'LAND' in c,
           map(str.upper, countries)),
    ''
)
...
from functools import reduce
countries = ['Estonia', 'Finland', 'Sweden', 'Denmark', 'Norway', 'Iceland']
phrase = reduce(lambda acc, c: acc + ', ' + c if c != countries[-1] else acc + ' et ' + c,
                countries[:-1], '') + ' sont des pays nord-européens.'
